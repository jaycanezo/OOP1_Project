...
public class Quest2Panel extends JPanel {
    ...
    public Quest2Panel(GameWindow game) {
        ...
        // Load a Nation 2 background for the quest
        try {
        ...
        addKeyListener(new KeyAdapter() {
            ...
            public void keyPressed(KeyEvent e) {
                ...
                // Allow returning to the story after the game ends
    ...
    public void startQuest() {
        ...
        isGameOver = false;
        isVictory = false;
        isStarting = true; // Set to true at the beginning
        leftPressed = false;
        rightPressed = false;
        arrows = new ArrayList<>();

        // Stop any running timers to be safe
    ...
    private void setupTimers() {
        // Main game loop (runs every 16ms for ~60 FPS)
        gameLoopTimer = new Timer(16, e -> {
            if (!isGameOver && !isStarting) { // Guard against running during intro
                updatePlayer();
                spawnAndMoveArrows();
                checkCollisions();
        ...
        countdownTimer = new Timer(1000, e -> {
            if (!isGameOver && !isStarting) { // Guard against running during intro
                timeLeft--;
                if (timeLeft <= 0) {
                    endQuest(true); // Survived 15 seconds!
                }
            }
        });

        // 3-Second Intro Timer
        ...
        introTimer.setRepeats(false); // Only run once per quest
    }
    ...
    private void spawnAndMoveArrows() {
        // Dynamic speed based on time left (Faster as time gets closer to 0)
        double baseSpeed = 5.0;
        double speedMultiplier = (15 - timeLeft) * 0.6; 
        double currentArrowSpeed = baseSpeed + speedMultiplier;

        // Spawn rate increases slightly as time goes down
        ...
        while (it.hasNext()) {
            ...
            if (a.y > getHeight()) {
                it.remove(); // Remove arrows that pass the screen
            }
        }
    }

    private void checkCollisions() {
        // Create a slightly smaller bounding box for the player to make dodging feel fair
    ...
    private void endQuest(boolean win) {
        ...
        if (player != null) {
            if (win) {
                ...
                player.setGold(Math.max(0, player.getGold() - 500)); // Prevent negative gold
            }
        }
        repaint();
    }
    ...
    protected void paintComponent(Graphics g) {
        ...
        // 3. Conditional Drawing: Intro vs Gameplay
        if (isStarting) {
            drawIntroOverlay(g2);
        } else {
            // Draw Arrows
    ...
    private void drawCustomArrow(Graphics2D g2, int x, int y) {
        ...
        arrowShape.moveTo(x + 7, y + 40); // Tip
        arrowShape.lineTo(x + 15, y + 25);
        arrowShape.lineTo(x + 10, y + 25);
        arrowShape.lineTo(x + 10, y);      // Top right
        arrowShape.lineTo(x + 4, y);       // Top left
        ...
        g2.setColor(Color.WHITE);
        g2.draw(arrowShape);
        g2.setColor(new Color(0, 0, 150)); // Reset color
    }
    ...
    private void drawResultOverlay(Graphics2D g2) {
        ...
        g2.setColor(new Color(181, 153, 110)); // Gold border
    ...
    // Inner class for falling arrows
    private class Arrow {
        int x;
        double y;
    ...
    private void drawIntroOverlay(Graphics2D g2) {
        // Darken background to emphasize the banner
